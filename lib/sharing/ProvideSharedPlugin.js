	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				// 这个方法核心的作用就是将 module 的 config 和 version 信息存在 resolvedProvideMap 中
				...
				// module hook 在 NormalModule 创建后触发
					...
					(module, { resource, resourceResolveData }, resolveData) => {
						...
						const { request } = resolveData;
						{
							...
							if (config !== undefined) {
								...
								// 将 resolveData cacheable 属性置为 false，暂时不知道 cacheable 属性的作用，后面再看
				...
				Array.from(
					...
					([resource, { config, version }]) =>
						new Promise((resolve, reject) => {
							...
							// 意思就是需要将这些配置的 shared 模块按入口 entry 配置的方式作为一个 module 去构建
